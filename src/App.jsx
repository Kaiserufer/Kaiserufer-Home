...
const MitarbeiterApp=({patienten,setPatienten,paesse,setPaesse,log,setLog,rechnungsNr,setRechnungsNr,einzel,setEinzel,urlaub,setUrlaub,teamEvents,setTeamEvents,schichten,setSchichten})=>{
  ...
  const deletePass=async(pid)=>{const pw=prompt("Passwort zum Löschen:");if(pw!=="Vogel"){if(pw!==null)alert("Falsches Passwort");setConfirmDelete(null);return;}const pass=paesse.find(p=>p.id===pid);if(pass){const pat=patienten.find(p=>p.id===pass.pat_id);await supabase.from("log").insert({id:genId(),pat_id:pass.pat_id,pass_id:pid,typ:"PASS_GELOESCHT",quelle:"INTERN",datum:new Date().toISOString(),notiz:`Pass gelöscht: ${getPassLabel(pass)} | HE:${pass.he_genutzt||0}/${pass.he_total||0} GA:${pass.bs_genutzt||0}/${pass.bs_total||0} | ${pass.rechnung||"–"} | ${pass.preis||0}€ | ${(pat?.vorname||"")} ${(pat?.nachname||"")}`});setLog(prev=>[...prev,{id:genId(),pat_id:pass.pat_id,pass_id:pid,typ:"PASS_GELOESCHT",quelle:"INTERN",datum:new Date().toISOString(),notiz:`Pass gelöscht`}]);}await supabase.from("paesse").delete().eq("id",pid);setPaesse(prev=>prev.filter(p=>p.id!==pid));setConfirmDelete(null);};
