...
const KaufModal=({selPat,onKauf,onClose})=>{
  ...
  const sP=()=>{if(passTyp==="INDIVIDUELL")onKauf("individuell",{name:passName||"Individuell",he:passHE,bs:passBS,datum:passDatum,rechnung:passRechnung.trim()},passPreis,"");else onKauf("pass",{typ:passTyp,he:passHE,bs:passBS},passPreis,passRechnung.trim(),passDatum);};
...
/* ═══ MITARBEITER APP ═══ */
const audit=async(aktion,bereich,details,pat_id,pass_id,quelle)=>{try{await fetch("/api/audit-log",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({aktion,bereich,details,pat_id,pass_id,quelle:quelle||"INTERN"})});}catch(e){}};
...
const MitarbeiterApp=({patienten,setPatienten,paesse,setPaesse,log,setLog,rechnungsNr,setRechnungsNr,einzel,setEinzel,urlaub,setUrlaub,teamEvents,setTeamEvents,schichten,setSchichten})=>{
  ...
  const checkAutoDeduct=async()=>{try{const r=await fetch("/api/pass-auto-deduct");const d=await r.json();const{data:np}=await supabase.from("paesse").select("*");const{data:nl}=await supabase.from("log").select("*");if(np&&nl){/* Manuell-Schutz: Pässe mit manuellen Log-Einträgen nicht auto-korrigieren */const manualPasses=new Set(nl.filter(l=>l.quelle==="MANUELL").map(l=>l.pass_id));for(const p of np){if(manualPasses.has(p.id))continue;const heLogs=nl.filter(l=>l.pass_id===p.id&&l.typ==="HAUPTEINHEIT").length;const heKorr=nl.filter(l=>l.pass_id===p.id&&l.typ==="KORREKTUR"&&(l.notiz||"").includes("HE")).reduce((s,l)=>{const m=(l.notiz||"").match(/\+(\d+)/);return s+(m?parseInt(m[1]):0);},0);const gaLogs=nl.filter(l=>l.pass_id===p.id&&l.typ==="BS").length;const gaKorr=nl.filter(l=>l.pass_id===p.id&&l.typ==="KORREKTUR"&&(l.notiz||"").includes("GA")).reduce((s,l)=>{const m=(l.notiz||"").match(/\+(\d+)/);return s+(m?parseInt(m[1]):0);},0);const expectedHE=Math.max(0,heLogs-heKorr);const expectedGA=Math.max(0,gaLogs-gaKorr);if(expectedHE>(p.he_genutzt||0)){await supabase.from("paesse").update({he_genutzt:expectedHE}).eq("id",p.id);p.he_genutzt=expectedHE;}if(expectedGA>(p.bs_genutzt||0)){await supabase.from("paesse").update({bs_genutzt:expectedGA}).eq("id",p.id);p.bs_genutzt=expectedGA;}}setPaesse(np);setLog(nl);}if(d.deducted?.length){const now=Date.now();const updates={};d.deducted.forEach(x=>{updates[x.patient]=now;});setAutoDeductRecent(prev=>({...prev,...updates}));fetchShoreCalendar();}}catch(e){}};
  ...
  const handleKaufFuerPat=async(pat,typ,info,preis,eigeneRechnung,datum,istAlt,bezahltStatus)=>{
    ...
    else if(typ==="pass"){rs=eigeneRechnung||genRechnung(await getRechnungsNr());const typKey=typeof info==="string"?info:info.typ;const pt=PASS_TYPES[typKey];const customHE=typeof info==="object"&&info.he!=null?info.he:pt.he;const customBS=typeof info==="object"&&info.bs!=null?info.bs:pt.bs;const alt=!!istAlt,bez=bezahltStatus!=null?bezahltStatus:false;const np={id:genId(),pat_id:pat.id,typ:typKey,he_total:customHE,he_genutzt:alt?customHE:0,bs_total:customBS,bs_genutzt:alt?customBS:0,preis:preis||0,rechnung:rs,bezahlt:bez,datum:ds,aktiv:!alt};await supabase.from("paesse").insert(np);setPaesse(prev=>[...prev,np]);}
  ...
  const deletePass=async(pid)=>{const pw=prompt("Passwort zum Löschen:");if(pw!=="Vogel"){if(pw!==null)alert("Falsches Passwort");setConfirmDelete(null);return;}const pass=paesse.find(p=>p.id===pid);if(pass){const pat=patienten.find(p=>p.id===pass.pat_id);const details=`Pass gelöscht: ${getPassLabel(pass)} | HE:${pass.he_genutzt||0}/${pass.he_total||0} GA:${pass.bs_genutzt||0}/${pass.bs_total||0} | ${pass.rechnung||"–"} | ${pass.preis||0}€ | ${(pat?.vorname||"")} ${(pat?.nachname||"")}`;await supabase.from("log").insert({id:genId(),pat_id:pass.pat_id,pass_id:pid,typ:"PASS_GELOESCHT",quelle:"INTERN",datum:new Date().toISOString(),notiz:details});setLog(prev=>[...prev,{id:genId(),pat_id:pass.pat_id,pass_id:pid,typ:"PASS_GELOESCHT",quelle:"INTERN",datum:new Date().toISOString(),notiz:`Pass gelöscht`}]);audit("PASS_GELOESCHT","PASS",details,pass.pat_id,pid);}await supabase.from("paesse").delete().eq("id",pid);setPaesse(prev=>prev.filter(p=>p.id!==pid));setConfirmDelete(null);};
  ...
  const heAbziehen=async(pass)=>{if(!pass||pass.he_genutzt>=pass.he_total)return;const prev={he_genutzt:pass.he_genutzt};const u={...pass,he_genutzt:pass.he_genutzt+1};const nl={id:genId(),pat_id:selPat.id,pass_id:pass.id,typ:"HAUPTEINHEIT",quelle:"INTERN",datum:new Date().toISOString(),notiz:"Haupteinheit"};await supabase.from("paesse").update({he_genutzt:u.he_genutzt}).eq("id",pass.id);await supabase.from("log").insert(nl);setPaesse(p=>p.map(x=>x.id===pass.id?u:x));setLog(p=>[...p,nl]);audit("HE_ABZUG","PASS",`${selPat.vorname} ${selPat.nachname}: HE ${prev.he_genutzt}→${u.he_genutzt} (Pass ${pass.rechnung||pass.id})`,selPat.id,pass.id);
    setUndoAction({msg:"Haupteinheit −1 bei "+selPat.vorname,undo:async()=>{await supabase.from("paesse").update(prev).eq("id",pass.id);await supabase.from("log").delete().eq("id",nl.id);setPaesse(p=>p.map(x=>x.id===pass.id?{...x,...prev}:x));setLog(p=>p.filter(l=>l.id!==nl.id));audit("HE_UNDO","PASS",`${selPat.vorname} ${selPat.nachname}: HE ${u.he_genutzt}→${prev.he_genutzt} (Rückgängig)`,selPat.id,pass.id);}});};
  const bsAbziehen=async(pass)=>{if(!pass||pass.bs_genutzt>=pass.bs_total||!bsNotiz.trim())return;const prev={bs_genutzt:pass.bs_genutzt};const u={...pass,bs_genutzt:pass.bs_genutzt+1};const nl={id:genId(),pat_id:selPat.id,pass_id:pass.id,typ:"BS",quelle:"INTERN",datum:new Date().toISOString(),notiz:bsNotiz.trim()};await supabase.from("paesse").update({bs_genutzt:u.bs_genutzt}).eq("id",pass.id);await supabase.from("log").insert(nl);setPaesse(p=>p.map(x=>x.id===pass.id?u:x));setLog(p=>[...p,nl]);setBsNotiz("");setBsModal(null);audit("GA_ABZUG","PASS",`${selPat.vorname} ${selPat.nachname}: GA ${prev.bs_genutzt}→${u.bs_genutzt} (${bsNotiz.trim()})`,selPat.id,pass.id);
    setUndoAction({msg:"Gruppenangebot −1 bei "+selPat.vorname,undo:async()=>{await supabase.from("paesse").update(prev).eq("id",pass.id);await supabase.from("log").delete().eq("id",nl.id);setPaesse(p=>p.map(x=>x.id===pass.id?{...x,...prev}:x));setLog(p=>p.filter(l=>l.id!==nl.id));audit("GA_UNDO","PASS",`${selPat.vorname} ${selPat.nachname}: GA Rückgängig`,selPat.id,pass.id);}});};
  const korrekturSpeichern=async()=>{if(!korrekturModal||korrekturAnzahl<1)return;const n=Number(korrekturAnzahl);const pass=korrekturModal;const upd=korrekturTyp==="HE"?{he_genutzt:Math.max(0,(pass.he_genutzt||0)-n)}:{bs_genutzt:Math.max(0,(pass.bs_genutzt||0)-n)};const nl={id:genId(),pat_id:selPat.id,pass_id:pass.id,typ:"KORREKTUR",quelle:"MANUELL",datum:new Date().toISOString(),notiz:`${korrekturTyp} +${n} zurück${korrekturGrund?` – ${korrekturGrund}`:""}`};await supabase.from("paesse").update(upd).eq("id",pass.id);await supabase.from("log").insert(nl);setPaesse(p=>p.map(x=>x.id===pass.id?{...x,...upd}:x));setLog(p=>[...p,nl]);audit("KORREKTUR","PASS",`${selPat.vorname} ${selPat.nachname}: ${korrekturTyp} +${n} zurück${korrekturGrund?` – ${korrekturGrund}`:""} (Pass ${pass.rechnung||pass.id})`,selPat.id,pass.id,"MANUELL");setKorrekturModal(null);setKorrekturAnzahl(1);setKorrekturGrund("");};
  ...
  const toggleBezahlt=async(pid)=>{const p=paesse.find(x=>x.id===pid);if(!p)return;await supabase.from("paesse").update({bezahlt:!p.bezahlt}).eq("id",pid);setPaesse(prev=>prev.map(x=>x.id===pid?{...x,bezahlt:!x.bezahlt}:x));const pat=patienten.find(x=>x.id===p.pat_id);audit("BEZAHLT_TOGGLE","PASS",`${pat?.vorname||""} ${pat?.nachname||""}: ${!p.bezahlt?"BEZAHLT":"OFFEN"} (Pass ${p.rechnung||pid}, ${p.preis||0}€)`,p.pat_id,pid);};
  ...
  const updatePassField=async(pid,field,val)=>{const pass=paesse.find(p=>p.id===pid);const old=pass?pass[field]:undefined;await supabase.from("paesse").update({[field]:val}).eq("id",pid);setPaesse(prev=>prev.map(p=>p.id===pid?{...p,[field]:val}:p));if(pass){const pat=patienten.find(p=>p.id===pass.pat_id);audit("FELD_GEAENDERT","PASS",`${pat?.vorname||""} ${pat?.nachname||""}: ${field} "${old}"→"${val}" (Pass ${pass.rechnung||pid})`,pass.pat_id,pid,"MANUELL");}};
  const updatePassEinheiten=async(pid,field,val)=>{const n=Math.max(0,parseInt(val)||0);const pass=paesse.find(p=>p.id===pid);if(!pass)return;const old=pass[field]||0;if(n===old)return;await supabase.from("paesse").update({[field]:n}).eq("id",pid);const diff=n-old;if(field==="he_genutzt"||field==="bs_genutzt"){const typ=field==="he_genutzt"?"HAUPTEINHEIT":"BS";if(diff>0){for(let i=0;i<diff;i++){const nl={id:genId(),pat_id:pass.pat_id,pass_id:pid,typ,quelle:"MANUELL",datum:new Date().toISOString(),notiz:`Manuell eingetragen`};await supabase.from("log").insert(nl);setLog(p=>[...p,nl]);}}else if(diff<0){const passLogs=log.filter(l=>l.pass_id===pid&&l.typ===typ).sort((a,b)=>(b.datum||"").localeCompare(a.datum||""));for(let i=0;i<Math.abs(diff)&&i<passLogs.length;i++){await supabase.from("log").delete().eq("id",passLogs[i].id);setLog(p=>p.filter(l=>l.id!==passLogs[i].id));}}}setPaesse(prev=>prev.map(p=>p.id===pid?{...p,[field]:n}:p));const pat=patienten.find(p=>p.id===pass.pat_id);audit("EINHEITEN_GEAENDERT","PASS",`${pat?.vorname||""} ${pat?.nachname||""}: ${field} ${old}→${n} (Pass ${pass.rechnung||pid})`,pass.pat_id,pid,"MANUELL");};
  ...
  const PassCard=({pk,isAlt})=>{
    ...
    return(<div style={{borderRadius:16,border:`1px solid ${T.cardBorder}`,background:isAlt?T.bgPale+"90":T.cream+"90",overflow:"hidden",marginBottom:12,opacity:isAlt?0.8:1}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 20px",borderBottom:`1px solid ${T.cardBorder}`,background:T.bgPale+"80",flexWrap:"wrap",gap:8}}>
        <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}><strong style={{fontFamily:"Georgia,serif",fontSize:17,color:T.oliveDark}}>Flossenpass</strong>{isAlt&&<Badge variant="cream" small>Aufgebraucht</Badge>}{pk.rechnung_pdf&&<a href={pk.rechnung_pdf} target="_blank" rel="noopener noreferrer" style={{padding:"3px 10px",borderRadius:8,fontSize:11,fontWeight:700,background:T.green+"20",color:T.green,textDecoration:"none",border:`1px solid ${T.green}30`}}>Rechnung PDF ↗</a>}</div>
