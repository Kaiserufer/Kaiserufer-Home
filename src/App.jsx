  ...
  return(<div className="resp-pad" style={{padding:28}}>
    ...
    {pendingPasses.length>0&&<div className="fade-in" style={{marginBottom:22}}>
      {pendingPasses.map(pp=>{
        ...
        return(<div key={pp.orderId} style={{borderRadius:18,background:"linear-gradient(135deg,#e46d73 0%,#d4555c 100%)",padding:"24px 28px",marginBottom:12,boxShadow:"0 4px 20px rgba(228,109,115,0.3)"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:14}}>
            <div style={{flex:1,minWidth:200}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}><span style={{fontSize:11,fontWeight:800,color:"rgba(255,255,255,0.7)",textTransform:"uppercase",letterSpacing:2}}>Neuer Flossenpass-Verkauf</span><span style={{background:"rgba(255,255,255,0.2)",color:"#fff",fontSize:11,fontWeight:700,padding:"3px 10px",borderRadius:20}}>{passDate}</span></div>
              <div style={{fontSize:22,fontWeight:700,color:"#fff",marginBottom:6}}>{pp.customer?.name||"Unbekannt"}</div>
              <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                <span style={{background:"rgba(255,255,255,0.2)",backdropFilter:"blur(4px)",color:"#fff",fontSize:13,fontWeight:700,padding:"5px 14px",borderRadius:24}}>{getPassName(pp.passType)}</span>
                <span style={{fontSize:15,fontWeight:700,color:"#fff"}}>{pp.price}€</span>
                {!pp.priceMatch&&<span style={{fontSize:13,color:"rgba(255,255,255,0.7)",fontWeight:600}}>statt {pp.standardPrice}€</span>}
                {invNum&&<span style={{fontSize:13,color:"rgba(255,255,255,0.8)",fontWeight:600}}>· RN{invNum}</span>}
              </div>
            </div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {pp.priceMatch?<button onClick={()=>confirmPassSale(pp)} style={{padding:"10px 22px",borderRadius:12,background:"#fff",color:"#e46d73",fontWeight:700,fontSize:14,border:"none",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.1)"}}>Bestätigen</button>:<button onClick={()=>setEditPass(isEdit?null:{orderId:pp.orderId,he:PASS_TYPES[pp.passType]?.he||0,bs:PASS_TYPES[pp.passType]?.bs||0,preis:pp.price,name:getPassName(pp.passType),rechnung:invNum?"RN"+invNum:(pp.invoiceNumber||""),datum:passDate})} style={{padding:"10px 22px",borderRadius:12,background:"#fff",color:"#e46d73",fontWeight:700,fontSize:14,border:"none",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.1)"}}>{isEdit?"Schließen":"Anpassen"}</button>}
              <button onClick={()=>dismissPassSale(pp)} style={{padding:"10px 22px",borderRadius:12,background:"rgba(255,255,255,0.15)",color:"#fff",fontWeight:600,fontSize:14,border:"2px solid rgba(255,255,255,0.3)",cursor:"pointer",backdropFilter:"blur(4px)"}}>Ignorieren</button>
            </div>
          </div>
          {isEdit&&<div style={{marginTop:18,paddingTop:18,borderTop:"1px solid rgba(255,255,255,0.2)",display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
            <div><label style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.7)",textTransform:"uppercase",display:"block",marginBottom:4,letterSpacing:1}}>Haupteinheiten</label><input type="number" min={0} value={editPass.he} onChange={e=>setEditPass({...editPass,he:Number(e.target.value)})} style={{...inp,background:"rgba(255,255,255,0.15)",color:"#fff",border:"1.5px solid rgba(255,255,255,0.3)"}}/></div>
            <div><label style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.7)",textTransform:"uppercase",display:"block",marginBottom:4,letterSpacing:1}}>Gruppenangebote</label><input type="number" min={0} value={editPass.bs} onChange={e=>setEditPass({...editPass,bs:Number(e.target.value)})} style={{...inp,background:"rgba(255,255,255,0.15)",color:"#fff",border:"1.5px solid rgba(255,255,255,0.3)"}}/></div>
            <div><label style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.7)",textTransform:"uppercase",display:"block",marginBottom:4,letterSpacing:1}}>Preis</label><input type="number" min={0} value={editPass.preis} onChange={e=>setEditPass({...editPass,preis:Number(e.target.value)})} style={{...inp,background:"rgba(255,255,255,0.15)",color:"#fff",border:"1.5px solid rgba(255,255,255,0.3)"}}/></div>
            <div><label style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.7)",textTransform:"uppercase",display:"block",marginBottom:4,letterSpacing:1}}>Name</label><input value={editPass.name} onChange={e=>setEditPass({...editPass,name:e.target.value})} style={{...inp,background:"rgba(255,255,255,0.15)",color:"#fff",border:"1.5px solid rgba(255,255,255,0.3)"}}/></div>
            <div><label style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.7)",textTransform:"uppercase",display:"block",marginBottom:4,letterSpacing:1}}>Rechnungsnummer</label><input value={editPass.rechnung} onChange={e=>setEditPass({...editPass,rechnung:e.target.value})} placeholder="z.B. RN123" style={{...inp,background:"rgba(255,255,255,0.15)",color:"#fff",border:"1.5px solid rgba(255,255,255,0.3)"}}/></div>
            <div><label style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.7)",textTransform:"uppercase",display:"block",marginBottom:4,letterSpacing:1}}>Datum</label><input type="date" value={editPass.datum} onChange={e=>setEditPass({...editPass,datum:e.target.value})} style={{...inp,background:"rgba(255,255,255,0.15)",color:"#fff",border:"1.5px solid rgba(255,255,255,0.3)"}}/></div>
            <div style={{gridColumn:"span 3",display:"flex",gap:8,justifyContent:"flex-end",marginTop:4}}><button onClick={()=>confirmPassSale(pp,{he:editPass.he,bs:editPass.bs,preis:editPass.preis,name:editPass.name,rechnung:editPass.rechnung,datum:editPass.datum})} style={{padding:"10px 22px",borderRadius:12,background:"#fff",color:"#e46d73",fontWeight:700,fontSize:14,border:"none",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.1)"}}>Bestätigen</button><button onClick={()=>{const cname=pp.customer?.name||"";const parts=cname.toLowerCase().trim().split(/\s+/).filter(p=>p.length>0);const pat=patienten.find(p=>{const full=`${p.vorname||""} ${p.nachname||""}`.toLowerCase();return parts.length>0&&parts.every(part=>full.includes(part));});if(!pat){alert("Patient nicht gefunden: "+cname);return;}setSelPat(pat);setView("akte");dismissPassSale(pp);}} style={{padding:"10px 22px",borderRadius:12,background:"rgba(255,255,255,0.15)",color:"#fff",fontWeight:600,fontSize:14,border:"2px solid rgba(255,255,255,0.3)",cursor:"pointer"}}>Kundenakte öffnen</button></div>
          </div>}
        </div>);
      })}
    </div>}
